...
comandos:
  init [--preset base|lgpd-educacao] [--agents claude|antigravity|opencode]
  ...
  plano <feature> [--agents claude|antigravity|opencode]
                      ...
                      · opencode: plano-execucao.md com instruções para o
                        agente opencode executar interativamente (sem headless)
...
function cmdInit(rootDir, flags) {
  ...
  if (flags.agents !== undefined) {
    ...
    if (agent !== 'claude' && agent !== 'antigravity' && agent !== 'opencode') {
      console.error(`--agents desconhecido: "${flags.agents}" (use: claude, antigravity, opencode)`);
      return 2;
    }
    const rotulo = agent === 'claude' ? 'Claude Code' : agent === 'opencode' ? 'OpenCode' : 'Antigravity';
    const dirAgente = agent === 'claude' ? '.claude' : agent === 'opencode' ? '.opencode' : '.agents';
    const destRel = path.join(dirAgente, 'skills', 'onp-spec-driven');
...
// vs skills/ para opencode); senão, o que estiver instalado no projeto;
// default: claude.
function detectarAgente(rootDir, flag) {
  if (flag !== undefined && flag !== true) {
    if (flag !== 'claude' && flag !== 'antigravity' && flag !== 'opencode') return { erro: `--agents desconhecido: "${flag}" (use: claude, antigravity, opencode)` };
    return { agent: flag };
  }
  ...
  // opencode: skilla embaixo de skills/<categoria>/onp-spec-driven/
  if (segmentos.includes('skills')) return { agent: 'opencode' };
  ...
  const temOpencode = existsSync(path.join(rootDir, '.opencode', 'skills', 'onp-spec-driven'));
  if (temOpencode && !temClaude && !temAg) return { agent: 'opencode' };
...
function cmdPlano(project, positional, flags) {
  ...
  } else if (plan.agent === 'opencode') {
    console.log(`  · o agente opencode executa as tarefas interativamente seguindo`);
    console.log(`    o plano-execucao.md — use TodoWrite para acompanhar o progresso`);
  } else {
