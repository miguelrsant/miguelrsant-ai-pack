...
//   opencode    → plano-execucao.md com instruções para execução manual
//                 pelo agente opencode (sem headless — execução interativa)
...
export function montarPlano(project, featureName, opts = {}) {
  const agent = opts.agent === 'antigravity' ? 'antigravity' : opts.agent === 'opencode' ? 'opencode' : 'claude';
...
export function renderPlanoMd(plan) {
  ...
  if (plan.agent === 'claude') {
    ...
    L.push('### ▶ Interativo — OpenCode');
    L.push('');
    L.push('O próprio agente OpenCode executa as tarefas interativamente,');
    L.push('usando **TodoWrite** para acompanhar o progresso e **git worktrees**');
    L.push('para isolar contextos. O agente executa as faixas em paralelo');
    L.push('quando possível, uma de cada vez, usando worktrees limpas.');
    L.push('');
    ...
    L.push('```');
    L.push('');
    L.push(`2. **O agente executa cada ${plan.faixas.length > 1 ? 'faixa em seu worktree' : 'tarefa'}**,`);
    L.push('   1 tarefa = 1 commit, marcando progresso com TodoWrite.');
    L.push('');
    ...
    if (plan.sequenciais.length) {
      L.push('');
      L.push('4. **Tarefas sequenciais** — o agente executa uma a uma:');
    }
    L.push('');
    L.push(`${plan.sequenciais.length ? '5' : '4'}. **Gate final** (exit 0 ou não está pronto):`);
    L.push('');
    L.push('```bash');
    ...
    L.push('```');
    L.push('');
    L.push(`${plan.sequenciais.length ? '6' : '5'}. **Acompanhe ao vivo** (opcional): \`onp-spec painel ${plan.feature}\` abre um`);
    L.push('   painel no navegador refletindo tasks.md, as provas do verify e o gate em');
    L.push('   tempo real enquanto o agente trabalha — sem digitar mais nada.');
  } else {
