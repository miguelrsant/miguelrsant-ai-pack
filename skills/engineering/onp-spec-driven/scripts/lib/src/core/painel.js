  ...
  est.projetos.forEach(function(p){
    ...
    p.execucoes.forEach(function(ex){
      ...
      if(ex.agent==='claude'){
        ...
        h+='<span class="sub" style="font-size:.75rem">'+(ex.agent==='opencode'?'OpenCode':'Antigravity')+': execução interativa — este painel só acompanha</span>';
      }
...
export function servirPainel({
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (req.method === 'POST' && url.pathname === '/executar') {
      ...
      if (ex.agent !== 'claude') {
        const nome = ex.agent === 'opencode' ? 'OpenCode' : 'Antigravity';
        res.writeHead(409);
        return res.end(`plano do ${nome}: execução interativa — este painel só acompanha`);
      }
