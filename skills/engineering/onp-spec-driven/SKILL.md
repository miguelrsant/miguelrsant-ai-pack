...
`plano <feature> [--agents claude|antigravity|opencode]` · `painel <feature> [--porta N] [--sem-abrir]` ·
...
- `onp-spec plano <feature> [--agents opencode|claude|antigravity]` agrupa
  tarefas de **arquivos disjuntos** em **faixas paralelas** — 1 faixa = 1 git
  worktree + 1 branch + 1 janela de contexto limpa — e gera artefatos em
  `.spec/features/<feature>/`. Os artefatos dependem do agente:
  - **todos os agentes**: `plano-execucao.md` — faixas, ondas, gestão de
    branches/commits e gate;
  - **claude apenas**: `executar-tarefas.sh` (executor headless com `claude -p`)
    e `plano-execucao.html` (visual interativo com botão);
  - **opencode/antigravity**: artefatos de execução manual/interativa (sem
    headless — siga as instruções do `plano-execucao.md`).
- **Apresente o plano ao usuário antes de executar**: resuma as faixas, diga
  onde estão os arquivos e ofereça a rota. No opencode, você mesmo implementa
  faixa a faixa usando worktrees, seguindo branches e commits do plano, e
  atualiza o progresso com TodoWrite.
- **Acompanhamento ao vivo (ofereça sempre)**: rode `onp-spec painel <feature>`
  ...
  e o estado de cada uma. Para agentes claude com headless, o painel também
  mostra o stream do modelo (ferramentas, raciocínio, custo) e botões de
  execução. Para opencode e antigravity, o painel é apenas **informativo** —
  o progresso é refletido pelo ledger conforme as tarefas são marcadas.
...
- **Falhou uma faixa?** No opencode, o próprio agente corrige e reexecuta a
  tarefa com um novo worktree limpo. No claude, `executar-tarefas.sh --faixa <id>`
  refaz só ela. No antigravity, abra um novo agente com o prompt da faixa.
...
- **"O que o modelo está fazendo agora?"** → o painel mostra o estado de cada
  tarefa e, para agentes claude com headless, o stream da sessão (ferramenta,
  raciocínio, saída, custo) ao vivo.
- **"Só uma faixa falhou, como refaço só ela?"** → no opencode, o próprio
  agente corrige e reexecuta a tarefa; no claude, `bash <baseDir>/executar-tarefas.sh --faixa <id>`;
  no painel aparece o botão ↻ onde disponível.
